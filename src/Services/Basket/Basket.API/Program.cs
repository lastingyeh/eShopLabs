    ...
    public class Program
    {
        ...
        private static IWebHost BuildWebHost(IConfiguration configuration, string[] args) =>
            WebHost.CreateDefaultBuilder(args)
                ...
                {
                    var ports = GetDeinedPorts(configuration);

                    opts.Listen(IPAddress.Any, ports.httpPort, listenOpts =>
                    {
                        listenOpts.Protocols = HttpProtocols.Http1AndHttp2;
                    });

                    opts.Listen(IPAddress.Any, ports.grpcPort, listenOpts =>
                    {
                ...
                .UseFailing(opts =>
                {
                    opts.ConfigPath = "/Failing";
                    opts.NotFilteredPaths.AddRange(new[] { "/hc", "/liveness" });
                })
                .UseStartup<Startup>()
